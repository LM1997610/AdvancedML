...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        regularisation = np.sum(abs(W1)**2) + np.sum(abs(W2)**2)
        ...
        grads['W2'] = 1/N * np.dot(a_2.T, last_layer_error) + 2*reg*W2
        grads["b2"] = 1/N * np.sum(last_layer_error, axis = 0)

        
        dz1 = W2.dot(last_layer_error.T).T * (z_2 > 0) * 1
    ...
    def train(self, X, y, X_val, y_val,
        ...
        for it in tqdm(range(num_iters),  position=0, leave=True):
            ...
            permutation = list(np.random.permutation(num_train))  # shuffled index of examples
    ...
    def predict(self, X):
        ...
        z_2 = np.dot(X, self.params["W1"]) + self.params["b1"]
        a_2 = np.maximum(0, z_2)
        z_3 = np.dot(a_2, self.params["W2"]) + self.params["b2"]

        softmax = np.exp(z_3)
        scores = np.divide(softmax.T, np.sum(softmax, axis = 1)).T
        y_pred = np.argmax(scores, axis = 1) # qui zero 
