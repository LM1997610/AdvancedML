...
class train_my_model():
    ...
    def __init__(self, config_diz, number :int):
        ...
        if torch.cuda.is_available():
            print('Using device:', self.device,  '- Type:', torch.cuda.get_device_name(0), "\n")
    ...
    def train(self, wandb, save_and_plot = True):
        ...
        for epoch in tqdm(range(self.n_epochs-1), position=0, leave=True):
            ...
            for cnt,batch in enumerate(self.data_loader):
                ...
                print("sequences_train.size:", sequences_train.size())
    ...
    def test(self, wandb, ckpt_path=None):
...
#size = torch.Size([2, 3, 10, 22, 1])

# Create a random tensor with values between 0 and 1
#random_tensor = torch.rand(size).float().to(go.device)

#import torch
#from torchviz import make_dot

#go = train_my_model(config_diz, number = 1)
#go.make_the_model()

#yhat = go.model(random_tensor) # Give dummy batch to forward()
